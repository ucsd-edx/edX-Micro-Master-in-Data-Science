...
def combineLogs(t_start,t_end,log_dir='/private/tmp'):
    """
    combineLogs reads the files generated by the spark monitor and returns a combined dataframe indexed by the 
    unix timestamp.
    input:
       t_start, t_end: the time range of interest
       log_dir: the directory where the monitor log files reside
    output: a pandas dataframe. Row index = time. Columns index=identifier of measurement.
    """
