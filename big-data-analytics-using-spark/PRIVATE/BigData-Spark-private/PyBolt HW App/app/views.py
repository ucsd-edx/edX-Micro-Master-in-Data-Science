...
@app.route('/killjob/<jobid>')
@login_required
def killjob(jobid):
    job = Job.query.get(jobid)
    if job:
        if job.user_id != g.user.id:
            return "Sorry, cannot kill job %s" % (jobid)
        payload = {"appId" : jobid}
        try:
            r = requests.post(app.config['SPARK_KILL_API'], data=payload)
            response = r.json()
            if int(response['status']) == 0:
                db.session.delete(job)
                db.session.commit()
                return "Job %s killed succesfully! :) . Please refesh the page." % (jobid)
            else:
                return "Sorry, cannot kill job %s. Either it has completed or it is still running." % (jobid)
        except Exception,e:
            print e
            return "Sorry, cannot kill job %s" % (jobid)
...
def getStats(hw):
    try:
        hw = int(hw)
        if hw > app.config['HOMEWORK'] or hw < 1:
            return "Invalid HW"
    except:
        return "Invalid HW"
    result = []
    jobs = Job.query.filter(Job.status=="final").filter(Job.homework==hw).group_by(Job.user_id).all()
    pids = [str(job.user_id) for job in jobs]

    cleared = []
    with open("cleared.csv","rb") as l:
        f = l.read()
        cleared = f.split()

    waitlist = []
    with open("waitlist.csv","rb") as l:
        f = l.read()
        waitlist = f.split()

    result.append("Total registered students: %s" % len(cleared))
    result.append("Total waitlisted students: %s<br><br>" % len(waitlist))
    cleared_submitted = sum([1 for i in cleared if i in pids])
    waitlist_submitted = sum([1 for i in waitlist if i in pids])
    result.append("Students who have made final HW %s submission: " % (hw))
    result.append("Registered: %s" % cleared_submitted)
    result.append("Waitlisted: %s<br><br>" % waitlist_submitted)



    jobs = Job.query.group_by(Job.user_id).all()
    pids = [str(job.user_id) for job in jobs]
    no_cl = sum([1 for i in cleared if i not in pids])
    no_wl = sum([1 for i in waitlist if i not in pids])
    result.append("Students who didn't even start playground submissions")
    result.append("Registered: %s" % no_cl)
    result.append("Waitlisted: %s<br><br>" % no_wl)
